...
class TestDataWrangling:
    ...
    def test_dw_3(self):
        ...
        assert df.query("year == 2017").inflation_index.iloc[0] == pytest.approx(1.251222, abs=0.001)

    def test_dw_4(self):
        inflated_loss_ratio = df.get("inflated_loss_ratio")
        assert inflated_loss_ratio is not None
        assert inflated_loss_ratio.max() == pytest.approx(0.554702, abs=0.001)
        assert df.query("year == 2017").inflated_loss_ratio.iloc[0] == pytest.approx(0.534934, abs=0.001)
